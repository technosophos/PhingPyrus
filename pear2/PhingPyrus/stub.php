...
if (version_compare(phpversion(), '5.3.1', '<')) {
    if (substr(phpversion(), 0, 5) != '5.3.1') {
        ...
        echo "PhingPyrus requires PHP 5.3.1 or newer.
";
        exit -1;
    }
}
...
try {
    ...
    echo "Cannot process PhingPyrus phar:
";
    echo $e->getMessage(), "
";
    exit -1;
}
function PhingPyrus_autoload($class)
{
    $class = str_replace(array('_', '\\'), '/', $class);
    if (file_exists('phar://' . __FILE__ . '/PhingPyrus-0.4.0/php/' . $class . '.php')) {
        include 'phar://' . __FILE__ . '/PhingPyrus-0.4.0/php/' . $class . '.php';
    }
}
spl_autoload_register("PhingPyrus_autoload");
$phar = new Phar(__FILE__);
$sig = $phar->getSignature();
define('PhingPyrus_SIG', $sig['hash']);
define('PhingPyrus_SIGTYPE', $sig['hash_type']);
